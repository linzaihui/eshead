(function() {
	...
	acx.augment(String.prototype, {
		...
		'escapeJS' : function() {
			var meta = {'"':'\\"', '\\':'\\\\', '/':'\\/', '\b':'\\b', '\f':'\\f', '\n':'\\n', '\r':'\\r', '\t':'\\t'},
				xfrm = function(c) { return meta[c] || "joey\\u" + c.charCodeAt(0).toString(16).zeroPad(4); };
...
(function( app ) {
	...
	data.BoolQuery = ux.Observable.extend({
		...
		addClause: function(value, field, op, bool) {
			...
			if (op=="sort")
			{
				this.search.sort[field.substring(field.indexOf(".")+1)]=value;
				return "s-"+field.substring(field.indexOf(".")+1);
			}
			var clause = this._setClause(value, field, op, bool);
			var uqid = "q-" + this.refuid++;
			this.refmap[uqid] = { clause: clause, value: value, field: field, op: op, bool: bool };
			...
			return { tag: "TABLE", children: []
				...
				.concat(data.map(function(row) {
					return { tag: "TR", data: { row: row }, cls: "uiTable-row", children: columns.map(function(column){
						return { tag: "TD", cls: "uiTable-cell", children: [ { tag: "DIV", text: limitText((row[column] || "").toString()), "title": (row[column] || "").toString()} ] };
					})};
				}))
			};
		}
...
(function( $, app, i18n ) {
	...
	ui.FilterBrowser = ui.AbstractWidget.extend({
		...
		_createFilters_handler: function(data) {
			...
			if (indexName.substring(indexName.length-1)=="*")
			{
				var prefix=indexName.substring(0, indexName.length-1);
				var names=[];
				...
				{
					names.sort();
					indexName=names[names.length-1];
				}
			}
			if (data[indexName]){
				for(var type in data[indexName].mappings) {
					scan_properties([type], data[indexName].mappings[type]);
				}
			}
		...
		_get_data_table: function() {
			var tabs=document.getElementById("es-body").children;
			var tab=undefined;
			...
			return tab.getElementsByClassName("uiTable-body")[0].getElementsByTagName("table")[0];
		},

		_download_html_handler: function () {
			var table=this._get_data_table();
			if (table===undefined)
				return;
			...
			{
				var cell=cells[i].getElementsByTagName("div")[0];
				cell.innerText=cell.title;
			}
			var wb = new Blob([table.outerHTML], {type: "text/html"});
            var eleLink = document.createElement('a');
            eleLink.download = "es-result.html"; // 文件名
            eleLink.style.display = 'none';
            eleLink.href = URL.createObjectURL(wb);
		...
		_download_handler: function () {
			...
			{
				var cell=cells[i].getElementsByTagName("div")[0];
				cell.innerText=limitText(cell.title,32767);
			}
			var wb = XLSX.utils.table_to_book(table);
			/* Export to file (start a download) */
			XLSX.writeFile(wb, "es-result.xlsx");
		},

		_search_handler: function() {
			var tab=document.getElementsByClassName("uiApp-headerMenuItem pull-left active");
			var indexName=this.config.index;
			if (indexName.length>3&&indexName.substring(indexName.length-3)=="-2*")
				indexName=indexName.substring(0, indexName.length-3);
			tab[0].firstChild.textContent=indexName;
			window.setTimeout(function(){
				var headerItem=$("#es-header");
				var headerItemHeight=headerItem.height();
				document.getElementById("es-body").style.paddingTop=headerItemHeight+"px";
			}, 1000);
		...
		_changeQueryField_handler: function(jEv) {
			...
			if(spec.type === 'match_all') {
				...
				ops = ["match", "sort", "term", "wildcard", "prefix", "fuzzy", "range", "query_string", "text", "missing"];
		...
		_main_template: function() {
			return { tag: "DIV", children: [
				...
				{ tag: "BUTTON", type: "button", text: i18n.text("General.DownloadExcel"), onclick: this._download_handler },
				{ tag: "BUTTON", type: "button", text: i18n.text("General.DownloadHTML"), onclick: this._download_html_handler },
				{ tag: "LABEL", children:
...
(function( $, app, i18n ) {
	...
	ui.IndexSelector = ui.AbstractWidget.extend({
		...
		_update_handler: function(data) {
			...
			var merged={};
			var pattern=/[-_]20[0-9]{2,}([-_][0-9]+)?$/;
			for(var i=0; i < index_names.length; i++) { 
				var name = index_names[i];
				var value=data.indices[name];
				var matched=pattern.exec(name);
				if (matched&&matched.length>0)
					name=name.substring(0, matched.index+2)+"*";
				merged[name]=value;
			}

			index_names = Object.keys(merged).sort();
			for(var i=0; i < index_names.length; i++) {
				var name = index_names[i];
				options.push(this._option_template(name, merged[name])); 
			}
